...
from validators.business_validator import validate_business_rules


def load_schema(filename):
    schema_file = filename.replace(".csv", ".yaml")
    schema_path = f"config/{schema_file}"
...
def merge_quarantine_dfs(dfs):
    ...
    # Remove None and empty dataframes
    valid_dfs = [
        df for df in dfs
    ...
    combined = (
        combined.groupby(
            list(combined.columns.difference(["error_reason"])),
            dropna=False
        )["error_reason"]
